...
app.post('/mbr/request_mortgage', function(request, response) {
	...
	if (request.body.mortgage_value == undefined) {
		missing.push("mortgage_value");
	}
	if (request.body.mlsID == undefined) {
		missing.push("mlsID");
	}
	if (missing.length != 0) {
		...
		collection.insertOne({
			...
			function(err, result) {
				if (err) {
					...
					return logAndRespond(request, response, "/mbr/request_mortgage", 201, JSON.stringify({
						mortID: new_mortID
					}));
				}
			}
...
app.post('/mbr/submit_insurance_quote', function(request, response) {
	...
	if (request.body.insured_value == undefined) {
		missing.push("insured_value");
	}
	if (request.body.deductible_value == undefined) {
		missing.push("deductible_value");
	}
	if (missing.length != 0) {
		...
		collection.updateOne({
			...
			function(err, result) {
				if (err) {
					...
					return logAndRespond(request, response, "/mbr/submit_insurance_quote", 400, "No mortgage found associated with name:" + request.body.name + ", and mortID: " + request.body.mortID + ".");
				}
			}
		);
	}
...
app.post('/mbr/submit_employer_info', function(request, response) {
	...
	if (request.body.employment_start == undefined) {
		missing.push("employment_start");
	}
	if (missing.length != 0) {
